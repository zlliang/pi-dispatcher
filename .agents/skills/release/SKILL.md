---
name: release
description: Releases pi-dispatcher to npm via Release Please. Use when asked to publish a new version or apply a patch/minor/major or explicit-version release.
---

# Release

pi-dispatcher publishes to npm via [Release Please](https://github.com/googleapis/release-please). Pushing to `main` runs `.github/workflows/release.yml`.
...
- The commit has real changes → put the footer on that same commit.
  ...
  ```bash
  git commit -m "docs: polish README" -m "Release-As: 0.1.1"
  ```

- You only need to trigger a release → use an empty commit.

  ```bash
  git commit --allow-empty -m "chore: trigger release 0.1.1" -m "Release-As: 0.1.1"
  ```
